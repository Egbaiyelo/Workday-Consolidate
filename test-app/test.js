...
process.stdin.on('readable', () => {
    ...
    try {
        ...
        while ((length = process.stdin.read(4)) !== null) {
            logDebug(`got length: ${length.readUInt32LE(0)}`);
    
            // Read message length    
            if (length) {
                const size = length.readUInt32LE(0);
                let messageBuffer = process.stdin.read(size);
                ...
                if (messageBuffer) {
                    ...
                    process.stdin.unshift(length);
                    logDebug('Unfulfilled - no message');
                    break;
                };
    
            } else {
                logDebug('Unfulfilled - no length');
                break;
            };
        }
